		...
		foreach ($this->session->data['compare'] as $key => $product_id) {
			...
			if ($product_info) {
				...
				$data['products'][$product_id] = array(
					...
					'price'        => $price,
					'special'      => $special,
					'description' => isset($product_info['mmos_shortdescr']) && $product_info['mmos_shortdescr'] != '' && $mmos_shortdescr_config['status'] == 1 && isset($mmos_shortdescr_config["pcompare"]) ?  $product_info['mmos_shortdescr'] : utf8_substr(strip_tags(html_entity_decode($product_info['description'], ENT_QUOTES, 'UTF-8')), 0, $this->config->get('config_product_description_length')) . '..',
