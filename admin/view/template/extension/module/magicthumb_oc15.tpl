...
$corePath = HTTP_SERVER.'view/image/magicthumb-opencart-module';
?>
  ...
  <form action="<?php echo $action; ?>" method="post" id="form">
	...
	<h3 class="settingsTitle">Module status</h3>
        ...
        <table class="params" cellspacing="0">
            <tr class="last">
                <th width="30%">
                    <label for="magicthumbsettingStatus">Enable/Disable module</label>
                </th>
                <td width="70%">
                    <select name="magicthumb_status">
                    <?php if ($this->config->get('magicthumb_status')) { ?>
        ...
        <table class="params" cellspacing="0">
            <tr>
                <th width="30%">
                    <label for="magicthumbsettingChache-path">Path to cache folder<br/>Relative for site base path</label>
                </th>
                <td width="70%">
            ...
            <tr class="back">
                <th width="30%">
                    <label for="magicthumbsettingTotal-items">Total items</label>
                </th>
                <td width="70%">
        ...
        <tr>
            <th width="30%">
                <label for="magicthumbsettingUnuser-items">Unused items</label>
            </th>
            <td width="70%">
        ...
        <tr class="last back">
            <th width="30%"><label for="magicthumbsettingEmpte-chache">Empty cache</label></th>
            <td width="70%">
                            ...
                            switch($s["type"]) {
                                case "array":
                                        ...
                                        foreach($s["values"] as $p) {
                                            ...
                                            $rButtons[strtolower($p)] = '<label><input type="radio" value="'.$p.'"'. ($s["value"]==$p?"checked=\"checked\"":"").' name="'.$profile.'_'.$s['id'].'" id="magicthumbsettings'. ucwords(strtolower($name)).$p.'">';
                                            $pName = ucwords($p);
                                            if(strtolower($p) == "yes")
                                ...
                                default:
                                    ...
                                    if (!isset($s["value"])) $s["value"] = $s["default"];
                                    $html .= '<input '.$width.' type="text" name="'.$profile.'_'.$s['id'].'" id="magicthumbsettings'. ucwords(strtolower($name)).'" value="'.$s["value"].'" />';
                                    break;
                            }
...
<?php
function trial_bage() {
if (!preg_match('/DEMO/is',@file_get_contents(DIR_CATALOG.'view/javascript/magicthumb.js'))) return;
echo '
