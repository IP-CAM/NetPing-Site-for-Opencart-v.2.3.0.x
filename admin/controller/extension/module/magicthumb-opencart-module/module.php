...
$GLOBALS['magicthumb_module_loaded'] = 'true'; // to fix boxes and pages conflict, I thunk we could find a better way in future

if (defined('HTTP_ADMIN')) {
    define ('MTOOLBOX_ADMIN_FOLDER_magicthumb',str_replace('catalog',preg_replace('/.*?([^\/]*)\/$/is','$1',HTTP_ADMIN),DIR_APPLICATION) . 'controller/'.$modulesPath.'/magicthumb-opencart-module/');
} else {
    define ('MTOOLBOX_ADMIN_FOLDER_magicthumb',DIR_APPLICATION . 'controller/'.$modulesPath.'/magicthumb-opencart-module/');
}

function magicthumb_LoadScroll($tool) {
    static $scroll = null;
    if($scroll === null) {
        $scroll = false;
        if($tool->params->checkValue('magicscroll', 'yes', 'product')) {
            require_once (MTOOLBOX_ADMIN_FOLDER_magicthumb.'magicscroll.module.core.class.php');
...
function magicthumb($content, $currentController = false , $type = false, $info = false) {

    
    $settings = $currentController->config->get('magicthumb_settings');
    $magicthumb_status = $settings['magicthumb_status'];
  

    if ($magicthumb_status != 0) {

        $tool = & magicthumb_load_core_class($currentController);
	...
	if ($tool->params->profileExists($type)) { //for main profiles
		...
		return $content;
            ...
            $content = magicthumb_parse_contents($content,$currentController);
            //if ($oldContent != $content) $content = magicthumb_set_headers($content);
            $content = magicthumb_set_headers($content); //TODO ?
	    ...
	    if ($type == 'product' && $tool->type == 'standard') {
                ...
                $tool->params->setProfile($type);
                require_once (MTOOLBOX_ADMIN_FOLDER_magicthumb.'magictoolbox.templatehelper.class.php');
                MagicToolboxTemplateHelperClass::setPath(MTOOLBOX_ADMIN_FOLDER_magicthumb.'templates');
                MagicToolboxTemplateHelperClass::setOptions($tool->params);
                $scroll = magicthumb_LoadScroll($tool);
                $html = MagicToolboxTemplateHelperClass::render(array(
                    'main' => magicthumb_getMainTemplate($GLOBALS['magictoolbox']['prods_info'],$type),
                    'thumbs' => magicthumb_getSelectorsTemplate($GLOBALS['magictoolbox']['prods_info'],$type),
                ...
                if (isset($m360[1])) {
                    $content = preg_replace('/(<a[^>]*?class=\"MagicThumb\"[^>]*?id=\"Magic[^\"]*Image\d+_product\")/is','<div class="m360container">'.$m360[1].'</div> $1 style="position: absolute; left: -10000px;"',$content , 1); //hide zoom main image & add 360 container
		...
		foreach ($GLOBALS['magictoolbox']['prods_info'] as $product) {
		    ...
		    $img = magicthumb_getThumb($src,'original',$product['product_id']);
		    $thumb = magicthumb_getThumb($src,$position.'-thumb',$product['product_id']);
		    $thumb2x = magicthumb_getThumb($src,$position.'-thumb2x',$product['product_id']);                
		...
		$GLOBALS['magictoolbox']['plinks'] = array();
                ...
                if (isset($GLOBALS['magictoolbox']['prods_info']['gallery_images']) && count($GLOBALS['magictoolbox']['prods_info']['gallery_images'])) {
                    foreach ($GLOBALS['magictoolbox']['prods_info']['gallery_images'] as $image) {
                        ...
                        $img = magicthumb_getThumb($src,'original',$pid);
                        $medium = magicthumb_getThumb($src,'thumb',$pid);
...
function magicthumb_set_headers ($content, $headers = false, $controller = false) {
    ...
    if (isset($GLOBALS["magictoolbox"]["magicthumb"])) {
        ...
        $plugin = magicthumb_load_core_class($controller);  
    } 
    ...
    $scroll = magicthumb_LoadScroll($plugin);
    ...
    if (!$plugin->params->checkValue('use-effect-on-category-page', 'No') || !$plugin->params->checkValue('use-effect-on-manufacturers-page', 'No') || !$plugin->params->checkValue('use-effect-on-search-page', 'No')) {//fix for category && manufacturers view switch
        ...
        $headers .= '<script type="text/javascript">
			...
			if(typeof(magicJS.Doc.je1) == "undefined") magicAddEvent = "jAddEvent";
					    ...
					    MagicThumb.update(\'MagicThumbImage\'+ zoomId, MagicThumbOptiMages[optId][id][\'original\'], MagicThumbOptiMages[optId][id][\'thumb\']);
                                        }
        ...
        if (!empty($GLOBALS['magictoolbox']['page_type']) && $GLOBALS['magictoolbox']['page_type'] == 'product') {
            ...
            <script type=\"text/javascript\" src=\"".$prefix."catalog/view/javascript/magicthumb_product.js\"></script>
            ";
        }
    

    if ($headers && $content && !isset($GLOBALS['magicthumb_headers_set'])) {

        if (preg_match("/components\/com_(ayelshop|aceshop|mijoshop)\/opencart\//ims",DIR_APPLICATION)) {
            $content = $headers.$content;
            $GLOBALS['magicthumb_headers_set'] = true;
        } else {
            $content = preg_replace('/\<\/head\>/is',"\n".$headers."\n</head>",$content,1,$matched);
        }

        if ($matched > 0) $GLOBALS['magicthumb_headers_set'] = true;
...
function &magicthumb_load_core_class($currentController = false) {
    if(!isset($GLOBALS["magictoolbox"])) $GLOBALS["magictoolbox"] = array();
    if(!isset($GLOBALS["magictoolbox"]["magicthumb"])) {
        /* load core class */
        require_once (MTOOLBOX_ADMIN_FOLDER_magicthumb.'magicthumb.module.core.class.php');
	...
	foreach($settings as $param_name => $param_value) {
		...
		if (!preg_match('/watermark/is',$param_name)) {
		    $GLOBALS["magictoolbox"]["magicthumb"]->params->setValue($matches[2],$param_value,$matches[1]);
		} else if (preg_match('/default_watermark/is',$param_name)) {
		    $GLOBALS["magictoolbox"]["magicthumb"]->params->setValue($matches[2],$param_value,$matches[1]);
		    $GLOBALS["magictoolbox"]["magicthumb"]->params->setValue(str_replace('default_','',$param_name),$param_value,'default');
		}
	    }
	} 
...
function magicthumb_parse_contents($content,$currentController) {
    ...
    if ($type == 'product') { //use only on product page
	  ...
	  if (is_array($opt) && count($opt)) {
		  ...
		  if (!empty($option_value)) {
		      $jsOptions[$opt_id][$option['product_option_value_id']]['original'] = magicthumb_getThumb('image/'.$option_value,'original');
		      $jsOptions[$opt_id][$option['product_option_value_id']]['thumb'] = magicthumb_getThumb('image/'.$option_value,'thumb');
		  }
	      }
	  }
      }
    ...
    if ($type == 'product') {
	    ...
	    $content = magicthumb_category_like_replace($products,$type,$content);
	} else {
	    $content = magicthumb_cut_boxes($products,$type,$content); //cut all product images
	}

    } 

    if (isset($jsOptions) && count($jsOptions)  && $content != $oldContent) $content = str_replace('</head>','<script type="text/javascript"> MagicThumbOptiMages = '.json_encode($jsOptions).'; </script></head>',$content);

    return $content;
}

function magicthumb_getProductParams ($id, $params = false) {
...
function magicthumb_getThumb($src, $size = null, $pid = null) {
    if($size === null) $size = 'thumb';
    require_once (MTOOLBOX_ADMIN_FOLDER_magicthumb.'magictoolbox.imagehelper.class.php');
    ...
    if (!isset($GLOBAL['imagehelper'])) {
        $GLOBAL['imagehelper'] = new MagicToolboxImageHelperClass($shop_dir, '/'.$image_dir.'magictoolbox_cache', $GLOBALS["magictoolbox"]["magicthumb"]->params, null, $url);
    }
    return preg_replace('/https?\:\/\//is','//',$GLOBAL['imagehelper']->create('/' . $src, $size, $pid));
}

function magicthumb_getMainTemplate($info,$type) {
    ...
    $id = $info['product_id'].'_'.$type;
     ...
     if ($type == 'product') {
        $js_id = '<script type="text/javascript"> idimg = "MagicThumbImage'.$id.'"; </script>';
    } else {
        $js_id = '';
    }
    
    $src = $image_dir.$info['image'];
    $img = magicthumb_getThumb($src,'original',$id);
    $thumb = magicthumb_getThumb($src,null,$id);
    $thumb2x = magicthumb_getThumb($src,'thumb2x',$id);                
...
function magicthumb_getSelectorsTemplate($info,$type) {
	...
	foreach ($images as $image) {
	    ...
	    $img = magicthumb_getThumb($src,'original',$id);
	    $medium = magicthumb_getThumb($src,null,$id);
	    $medium2x = magicthumb_getThumb($src,'thumb2x',$id);
	    $thumb = magicthumb_getThumb($src,'selector',$id);
	    $thumb2x = magicthumb_getThumb($src,'selector2x',$id);                
...
function magicthumb_set_params_from_config ($config = false) {
    if ($config) {
        $plugin = $GLOBALS["magictoolbox"]["magicthumb"];
...
function magicthumb_use_effect_on(&$tool) {
...
function magicthumb_category_like_replace ($products, $type, $content) {
	...
	if (isset($matches[1])) {
        ...
        $content = preg_replace('/<a[^<]*?<img[^>]*?id=\"'.$id.'\"[^<]*?<\/a>/is',magicthumb_getMainTemplate($product,$type),$content);
    }
        
    return $content;
}

function magicthumb_cut_boxes ($products,$type,$content) {
